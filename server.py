...
def broadcast(mensaje, fuente_conn):
    for conn in clientes.values():
        if conn != fuente_conn:  # enviar el mensaje de vuelta al cliente origen
            conn.sendall(mensaje)  # Enviar el mensaje a los demás clientes

# Manejar la conexión con un cliente
def manejar_cliente(conn, addr):
    print(f"[SERVER] Conectado satisfactoriamente con {addr}")
    clientes[addr] = conn  # Añadir el cliente al diccionario
    print(f"[SERVER] Clientes conectados: {len(clientes)}")
        ...
        while True:
            try:
                ...
                if MESSAGE_DELIMITER in recibido:
                    ...
                    print(f"[SERVER] {addr} dice: {mensaje}")
                    ...
                    if mensaje.lower() == "logout":  # Finalizar la conexión si el cliente envía "logout"
                        ...
                        conn.sendall((respuesta).encode('utf-8'))  # Enviar respuesta al cliente
    ...
    print(f"[SERVER] Desconectando {addr}")
...
def iniciar_servidor():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as servidor:
        ...
        print(f"[SERVER] Escuchando en el puerto {TCP_PORT}")
